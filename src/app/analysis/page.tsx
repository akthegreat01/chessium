...
import { RotateCw, Copy, Download, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, RefreshCcw, Check, Undo2, Share2, Home as HomeIcon } from 'lucide-react';
...
const BoardEditor = dynamic(() => import("@/components/BoardEditor"), { ssr: false });
...
function AnalysisContent() {
  ...
  useEffect(() => {
    ...
    // Handle modes from query params
    const mode = searchParams.get('mode');
    if (mode === 'ai') setShowBotModal(true);
    if (mode === 'editor') setShowEditor(true);
  }, [loadSavedGames, checkDailyStreak, searchParams]);
  ...
  }, [goBack, goForward, goToMove, flipBoard, toggleSound, showHintMove, hideHint, showHint, history.length, showEditor]);
  ...
  const handleEditorAnalyze = (fen: string) => {
    loadFen(fen);
    setShowEditor(false);
  };
  ...
  return (
    <>
      {showEditor && <BoardEditor onClose={() => setShowEditor(false)} onAnalyze={handleEditorAnalyze} />}
      <NotificationToast />
      <CelebrationOverlay />
          ...
          <div className="flex items-center justify-between mt-1.5 px-0.5 gap-2 w-full">
            ...
            <div className="flex items-center gap-0.5 bg-white/[0.03] rounded-lg p-0.5 border border-white/[0.05]">
              ...
              <button onClick={() => { window.location.href = '/'; }}
                className="p-1.5 md:p-2 rounded-md hover:bg-white/10 transition-all active:scale-90 group" title="Return Home">
                <HomeIcon className="w-4 h-4 text-gray-400 group-hover:text-white transition-colors" />
              </button>
              <div className="w-px h-5 bg-white/[0.08] mx-0.5 hidden sm:block" />
              <SettingsPanel />
            </div>
  ...
  return (
    <Suspense fallback={
      <div className="flex-1 flex items-center justify-center">
        <div className="text-gray-500 font-mono text-sm tracking-widest animate-pulse">LOADING ENGINE...</div>
      </div>
    }>
      <AnalysisContent />
    </Suspense>
