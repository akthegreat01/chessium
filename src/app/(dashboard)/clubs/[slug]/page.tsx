...
export default async function ClubDashboardPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (isOwner && !isMember && user) {
    const { data: profile } = await supabase.from('profiles').select('username, chess_com_username, lichess_username').eq('id', user.id).single();
    ...
    members.push({
      ...
      lichessUsername: profile?.lichess_username || null
    });
  }
